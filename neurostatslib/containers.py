...
class MatStruct(UserDict):
    ...
    def __getattr__(self, key):
        if key in self.data:
            ...
            elif isinstance(self.data[key], list) and isinstance(self.data[key][0], dict):
...
class MatCell:
    def __init__(self, data, name="root"):
        ...
        self.name = name
        self.containers = [MatStruct(d, name+"["+str(k)+"]") for k,d in enumerate(data)]
    ...
    def __repr__(self):
        cls = self.__class__
        template = "%s %s.%s with length %d" % (self.name, cls.__module__, cls.__name__, len(self.containers))
        # if len(self.fields):
        #     template += "\nFields:\n"
